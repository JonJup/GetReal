...
org_names <- names(files) %>% stringr::str_split(string = ., pattern = "\\.", n = 2) %>% sapply(function(x) x[[1]]) 
...
for (i in 2:length(files))
{
          ...
          CATCHMENTS <- rgdal::readOGR(dsn = files[i], layer = "CATCHMENTS")
          catchments <- st_as_sf(CATCHMENTS)
          st_write(catchments, paste0(org_names[i], ".gpkg"))
}
