...
#TODO sa plots at each level  
#TODO complete taxa lists 
#TODO new seasonal maps 
...
We derived typical assemblages using the commonness of each taxon a respective river type. 
Here, the commonness of a taxon refers to the fraction of sampling sites in a river type that harbored that taxon.
This is conceptually equal to the $B$ parameter used in for the Species Indicator Value [@Caceres2009; @Dufrene1997]. 
We also considered the second parameter of that statistic, the specificity $A$ which refers to the fraction of occurences of a taxon that fall within one river type. 
...
A taxon belongs to the TA of a river type if it is more common that some threshold, which depends on the taxonomic level of the taxon. 
The threshold for species is 0.20, for genera 0.33 and for family or lower resolutions 0.55. 
In addition to the typical assemblages we also derived lists of specific taxa. 
To be included in these the specificity had to be above 0.9 for a species, 0.7 for genus or 0.55 for family or lower taxonomic levels. 
...
This highlights that there must be several very common taxa, which is likely to some degree also the result of our extensive harmonization efforts. 

Plots for each taxon level separately are available for [macroinvertebrates](https://github.com/JonJup/GetReal/tree/master/002_working_package_02/001_community_data/002_combined/002_invertebrates/004_plots/002_zwischenbericht) and diatoms . 
...
After assessing the redundancy we joined the two river types with the highest redundancy, if that redundancy was above 0.75, and computed the new TAs. 
This procedure was iterated until no redundancies were above 0.75.
...
The river types 8 to 11 are mid-altitude (200 - 800 m.a.s.l.) differing in size and geology. 
...
The two river types RT15 and RT16 are high altitude rivers two of thee types of high altitude rivers. 
...
In diatoms,  the river types 1, 2, 4, 5, 8, 16, 17, 18, and 19 are all combined into one large cluster, henceforth RT_large. 
This clustering is hard to explain, as it comprises very different river types while at the same time omitting seemingly similar ones. 
Such a clustering could be the result of a bad fit between river typology and biological data. 
...
In all macroinvertebrate TAs, genus is the prevalent taxonomic level, followed by families or lower taxonomic levels and lastly species (figure \@ref(fig:plot-mzb-n-taxa)A). 
...
`r mzb_min_rt` was the smallest assemblages with `r mzb_min_in` taxa and `r mzb_max_rt` was the most taxa rich assemblage with `r mzb_max_in` taxa. 
...
Note that RT3 or a combined river type including it (RT2_3) had the most taxa rich TA for both taxa.  
...
<!-- Discuss PCoA Plots --> 
  <!-- MZB -->
  The first axis of the macroinvertebrate PCoA separates the TAs into two groups: i) the large cluster 8-11_+_15_+_16, RT12, RT14, RT18, RT2_3 and ii) RT4_5 and RT1.
  ...
  However the cumulative relative eigenvalues of the first two principal components is only a little higher than half, which means that there is considerable varation in the distance matrix      that is not displayed in this PCoA 

  <!-- Diatoms -->
  In the PCoA of diatom TAs we can see that river types that should be similar to those included in the large cluster are close to the latter (i.e. RT9 and RT3).
  The two river types with strong influence of organic soils (RT6 and RT12) are distinct from each other and from the larger group. 
  Previous studies have already indicated, that there is strong turnover in diatom communities along gradients of organic matter [@Kelly1995;@coring1999situation; @Hering2006].
...
In most river types the differences between seasons are not large which is corroborated by stress values above 0.2.
