quiet_sim_mat <- function (data, method, diag = TRUE, upper = TRUE) 
{
        start.time <- Sys.time()
        stopifnot(data >= 0, data <= 1, method %in% c("Jaccard", 
                                                      "Sorensen", "Simpson", "Baroni"))
        ...
        for (ind in inds) {
                ...
                sim.mat[ind[1], ind[2]] <- fuzSim(x = data[, row], y = data[, 
                                                                            col], method = method)
                # if (pair == quarter) 
                #         message("25% done...")
                # if (pair == half) 
                #         message("50% done...")
                # if (pair == threequarters) 
                #         message("75% done...")
                # if (pair == n.pairs) 
                #         message("Finished!")
        }
