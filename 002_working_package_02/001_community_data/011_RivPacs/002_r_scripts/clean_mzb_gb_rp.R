...
setwd(here("011_RivPacs"))

sites <- read_excel(path = "01_OriginalData/Sites.xlsx") %>%
        setDT
taxa <- read_excel(path = "01_OriginalData/Taxa (Raw Data).xlsx") %>%
        setDT
data = taxa[sites, on = "Site ID"]
names(data)[14] <- "name"
...
data2[, abundance := data$`Num Ab (Full)`]
data2[, season := stringr::str_to_lower(season)]
saveRDS(data2, "03_FinalData/01_200108_data_before_taxon_clean_RIVPACS.RDS")
data2 = readRDS("03_FinalData/01_200108_data_before_taxon_clean_RIVPACS.RDS")
...
saveRDS(classification.object, "03_FinalData/02_191211_classification.object_RIVPACS.RDS")
classification.object = readRDS("03_FinalData/02_191211_classification.object_RIVPACS.RDS")
...
for (i in 259:(nrow(taxontable) + 1)) {
...
saveRDS(taxontable, "03_FinalData/03_191211_initial_taxon_clean_RIVPACS.RDS")
taxontable = readRDS("03_FinalData/03_191211_initial_taxon_clean_RIVPACS.RDS")
        
# now enter new rows to clean_diatoms and clean_gbif_errors
taxontable[order != "NA", clean := T]
taxontable[clean == F, taxon] %>% sort
        
saveRDS(taxontable, "03_FinalData/04_191211_post_correction_taxontable_RIVPACS.RDS")
taxontable = readRDS("03_FinalData/04_191211_post_correction_taxontable_RIVPACS.RDS")
...
saveRDS(level_list, "~/01_Uni/03_GetReal/002_WP_02/001_Community Data/012_Christian Feld/03_FinalData/08_2020-05-14_level_list_mzb_STARS.RDS")


# 04. Final touches -------------------------------------------------------

unique_sites = unique(data4$site)
unique_dates = unique(data4$season)[c(2,3,1)]
...
data4[,gr_sample_id := paste0("site_", site_id2, "_date_", date_id2,"_mzb_rivpacs")]
...
data6 = data5[, list(
  gr_sample_id,
  original_site_name = site.Site.ID,
  date = NA,
  year,
  season,
  site_id,
...
st_write(final, "03_FinalData/09_200513_MZB_Rivpacs_high_phyla.gpkg")   
saveRDS(final,  "03_FinalData/09_200513_MZB_Rivpacs_high_phyla.RDS")   
