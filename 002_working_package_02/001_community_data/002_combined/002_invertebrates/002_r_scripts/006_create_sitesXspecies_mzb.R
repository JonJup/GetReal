...
# date used: ..., 01.07.20, 03.07.20 + 14.09
#Jonathan Jupke 
...
mzb  = readRDS("003_processed_data/004_2020-07-03_mzb_data1585_low_impact.RDS")
setDT(mzb)
...
# all  
# -- rare taxa -- #
rare_id_all_spe      <- which(colSums(x = bio2_all_spe[,-1]) < 5) + 1
rare_id_all_gen      <- which(colSums(x = bio2_all_gen[,-1]) < 5) + 1
rare_id_all_foh      <- which(colSums(x = bio2_all_foh[,-1]) < 5) + 1
...
for (i in seq_along(bio2_files)) {
        ...
        saveRDS(
                object = save.file,
                file = paste0("003_processed_data/001_speciesXsites_tables/",
                              Sys.Date(),
                              "_",
                              save.name)
        )
