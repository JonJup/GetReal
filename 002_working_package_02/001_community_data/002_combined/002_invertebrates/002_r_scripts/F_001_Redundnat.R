...
redundant <- function(x, data){
        riv_x      <- rt_vector[x]
        riv_x_taxa <- data[group == riv_x, unique(taxon)]
        ...
        if (length(max_id) > 1) {
                ...
                print(paste(riv_x, "is most similiar is", max_id, "with", max_red, "% overlap"))
        } else {
                print(paste(riv_x, "is most similiar to" ,names(max_id), "with", max_red, "% overlap"))
        }
}