...
#pacman::p_load(sf, twinspanR, vegan, dplyr, fuzzySim, data.table, magrittr, purrr)
...
mzb  = readRDS("004_2020-03-18_mzb_data_low.RDS")
setDT(mzb)
...
unique(mzb$species) %>% 
        sort %>% 
        base::subset(stringr::str_detect(., pattern = "^E")) %>% 
...
unique(mzb$species) %>% 
        sort %>% 
        base::subset(stringr::str_detect(., pattern = "^Z")) %>% 
        str_split_fixed(pattern = " ", n = 2) %>% 
        .[,2] %>% 
        unique
...
unique(mzb$species) %>% 
        sort %>% 
        base::subset(stringr::str_detect(., pattern = "^A")) %>% 
        .[60:70]


mzb[species == "Molanna angusta", species := "Molanna angustata"]
mzb[species == "Notonectidae", c("species", "genus") := NA]
mzb[species == "Coproica", species := NA]

# genus 
unique(mzb$genus) %>% sort %>% base::subset(stringr::str_detect(., pattern = "^Z"))

mzb[genus == "Ferrisia", genus := "Ferrissia"]


# family 
unique(mzb$family) %>% sort %>% base::subset(stringr::str_detect(., pattern = "^Z"))

mzb[family == "Asteraceae", unique(genus)]
mzb[genus == "Dugesia", unique(family)]
mzb[genus == "Dugesia", unique(order)]
mzb[genus == "Dugesia", c("family", "order") := .("Dugesiidae", "Tricladida")]
mzb[family == "Cordulegastridae", family := "Cordulegasteridae"]
mzb[family == "Fabaceae", unique(genus)]
mzb[genus == "Ormosia", unique(family)]
mzb[genus == "Ormosia", unique(order)]
...
mzb[family == "Malvaceae", unique(genus)]
mzb[genus == "Sphaeroma", unique(family)]
mzb[genus == "Sphaeroma", unique(order)]
mzb[genus == "Sphaeroma", c("family", "order") := .("Sphaeromatidae", "Isopoda")]

mzb[family == "Rubiaceae", unique(genus)]
mzb[genus == "Lepidostoma", unique(family)]
mzb[genus == "Lepidostoma", unique(order)]
mzb[genus == "Lepidostoma", c("family", "order") := .("Lepidostomatidae", "Trichoptera")]
mzb[family == "Rutaceae", unique(genus)]
mzb[genus == "Limonia", unique(family)]
mzb[genus == "Limonia", unique(order)]
mzb[genus == "Limonia", c("family", "order") := .("Limoniidae", "Diptera")]
mzb[family == "Scatophagidae", family := "Scathophagidae"]
mzb[family == "Sphaeridae", family := "Sphaeriidae"]
mzb[family == "Stratiomydae", family := "Stratiomyidae"]
...
## order -- 
unique(mzb$order) %>% sort %>% base::subset(stringr::str_detect(., pattern = "^Z"))

mzb[order == "Anostraces", order := "Anostraca"]
mzb[genus == "Anisoptera", c("family", "order") := .("Libellulidae", "Odonata")]

mzb[order == "Perciformes", order := "Diptera"]
mzb[order == "Pulmonata", order := NA]

# 02. Subset  --------------------------------------------------------------
mzb$gr_sample_id %<>% as.character()
general_subset <- mzb[(is.na(year) | year >= 2000)]
general_subset[,gr_sample_id := as.character(gr_sample_id)]
...
for (i in seq_along(bio2.files)) {
        save.name = paste0(bio2.files[i], ".RDS")
