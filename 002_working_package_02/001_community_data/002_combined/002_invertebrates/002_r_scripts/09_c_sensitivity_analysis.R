...
for (i in 1:in_n) {
        ...
        dt_fol_a = dt_fol[A > in_v[i] * 0.77^2]
        dt_fol_b = dt_fol[B > in_v[i] * 1.65^2]
...
if (save) {
        saveRDS(dt_a_out, file.path(dir_pd, paste0("07_", Sys.Date(),"_sensitivity_parameter_a_50steps.RDS")))
        saveRDS(dt_b_out, file.path(dir_pd, paste0("07_", Sys.Date(),"_sensitivity_parameter_b_50steps.RDS")))   
}
