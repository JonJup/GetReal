### ----------------------------- ### 
# --- Clean MZB Sandin/ Kahlert --- #
### ----------------------------- ### 

## 25.10.19
## GR WP2 
## Here I clean and harmonize the biological data from Maria Kahlert and Leonard Sandin.
...
setwd(here("014_Sandin_Kahlert//"))

samples = readxl::read_excel("01_OriginalData/CompleteTaxalist_AQEM_Site_info.xls", skip = 2) %>%
  setDT
sites = readxl::read_excel("01_OriginalData/CompleteTaxalist_AQEM_Site_info.xls", sheet = 2) %>% 
  setDT
...
# data are only collected at family level though. I want to maintaim them but
# their second column is also empty. 

samples2 = samples[!(str_detect(samples$taxon, "[A-Z]{2,}") | 
                       taxon %in% c("[Kl:Turbellaria]", 
                                    "Turbellaria",
                                    "Nematoda",
                                    "[Kl:Nematoda]",
                                    "[Kl:Oligochaeta]",
...
samples2[taxon_supp %in% c("Gen. sp.", "sp.", "Gen. sp. Lv.", "sp. Lv.", "sp. Ad."), 
         taxon_supp := ""]

samples2[, taxon_clean := str_trim(paste(taxon,taxon_supp))]

samples3 = samples2[,.SD, .SDcols = 7:157]
samples3 = samples3[-1]

site_names <- colnames(samples3)[-151]
samples4 = data.table::melt.data.table(samples3, id.vars = c("taxon_clean"))
samples4 = samples4[value != 0]
...
sites2[,date := dmy(date)]
sites2[,c("year","season") := 
         list(
            year(date),
...
samples4$id = as.character(samples4$variable)

data  = left_join(samples4, 
                  sites2) %>% 
        setDT

data$taxon = unlist(data$taxon)
data[, date := ymd(date)]


saveRDS(data, "03_FinalData/01_191211_data_before_taxon_clean_mzb_Leonard_Sandin.RDS")
data = readRDS("03_FinalData/01_191211_data_before_taxon_clean_mzb_Leonard_Sandin.RDS")
# 03. Taxonomic Cleaning ------------------------------------------------------

TU = sort(unique(data$taxon))

classification.object = classification(TU, db = "gbif")
saveRDS(classification.object, "03_FinalData/02_191211_classification.object_mzb_Leonard_Sandin.RDS")
classification.object = readRDS("03_FinalData/02_191211_classification.object_mzb_Leonard_Sandin.RDS")
        ...
        ifelse(
          ...
          ifelse(
            ...
            ifelse("family" %in% tax[[1]][, 2],
                   ...
                   paste(tax[[1]][1][nrow(tax[[1]]),], "is an" ,tax[[1]][nrow(tax[[1]]),2]))
          )
        ),
        fill = T
      )
...
## -- used for cleaning -- ## 
taxontable[clean == F] %>% nrow
taxontable[order != "NA", clean := T]
taxontable[clean == F,taxon]

saveRDS(taxontable, "03_FinalData/04_191211_post_correction_taxontable_mzb_Leonard_Sandin.RDS")
taxontable = readRDS("03_FinalData/04_191211_post_correction_taxontable_mzb_Leonard_Sandin.RDS")

data3 = left_join(data,
                  taxontable, 
                  by = "taxon") %>% 
        setDT
...
saveRDS(data4, "03_FinalData/05b_200514_final_taxon_join_mzb_Leonard_Sandin_high_phyla.RDS")
...
saveRDS(level_list, "~/01_Uni/03_GetReal/002_WP_02/001_Community Data/014_Sandin_Kahlert/03_FinalData/08_200514_level_list_mzb_Leonard_Sandin.RDS")
...
unique_sites = unique(data4$id)
unique_dates = unique(data4$date) %>% sort

data4[, c("site_id", "date_id") := list(
        map_int(data4$id, ~ which(unique_sites == .x )),
...
data4[,gr_sample_id := paste0("site_", site_id2, "_date_", date_id2,"_mzb_Leonard_Sandin")]
...
st_write(final, "03_FinalData/09_200514_MZB_Leonard_Sandin_high_phyl.gpkg")   
saveRDS (final, "03_FinalData/09_200514_MZB_Leonard_Sandin_high_phyl.RDS")   
...
# make spatial object 
mzb_sw_sk.sf = st_as_sf(data4, coords = c("y.coord", "x.coord"), crs = data4$EPSG[1])

# test on map 
test1 = mzb_sw_sk.sf %>% setDT

test1 = unique(test1, by = "site")
...
# save to file 
st_write(mzb_sw_sk.sf, "03_FinalData/MZB_SW_SK_all.gpkg")
