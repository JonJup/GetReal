...
samples = readxl::read_excel("01_OriginalData/MI_data_KLHuttunen.xlsx") %>% 
        setDT
sites = readxl::read_excel("01_OriginalData/MI_data_KLHuttunen.xlsx", sheet = 2) %>% 
        setDT

# 02. Clean Data  ---------------------------------------------------------

samples2 = melt(samples, id.vars = c("Site", "Year"), value.name = "abundance", 
             variable.name = "taxon")

samples2 = samples2[abundance != 0]

# This is stated in Huttunen et al 2017: Habitat connectivity and in-stream vegetation control temporal variability of benthic invertebrate communities
samples2$season = "autumn"
# I the paper it is also stated that all sites are pristine 
sites[, pristine := 1]

data = samples2[sites, on = "Site"]
data$taxon = as.character(data$taxon)

data2 = data

saveRDS(data2, "03_FinalData/01_191211_data_before_taxon_clean_Kaisa-Leena_Huttunen.RDS")
...
saveRDS(classification.object, "03_FinalData/02_191211_classification.object_Kaisa-Leena_Huttunen.RDS")
classification.object = readRDS("03_FinalData/02_191211_classification.object_Kaisa-Leena_Huttunen.RDS")
...
saveRDS(taxontable, "03_FinalData/03_191211_initial_taxon_clean_Kaisa-Leena_Huttunen.RDS")
taxontable = readRDS("03_FinalData/03_191211_initial_taxon_clean_Kaisa-Leena_Huttunen.RDS")

# now enter new rows to clean_diatoms and clean_gbif_errors
taxontable[order != "NA", clean := T]
taxontable[clean == F, taxon] %>% sort

saveRDS(taxontable, "03_FinalData/04_191211_post_correction_taxontable_Kaisa-Leena_Huttunen.RDS")
...
saveRDS(data4, "03_FinalData/05b_200514_final_taxon_join_mzb_Kaisa-Leena_Huttunen_high_phyla.RDS")
...
saveRDS(level_list, "~/01_Uni/03_GetReal/002_WP_02/001_Community Data/015_KaisaLenaHuttunen/03_FinalData/08_2020-05-14_level_list_mzb_KLH.RDS")
...
st_write(final, "03_FinalData/09_200514_MZB_Kaisa-Leena_Huttunen_high_phylum.gpkg")   
saveRDS (final, "03_FinalData/09_200514_MZB_Kaisa-Leena_Huttunen_high_phylum.RDS")   
