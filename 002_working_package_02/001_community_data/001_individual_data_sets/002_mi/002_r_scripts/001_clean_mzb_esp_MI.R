...
# 14.10.19 
# Cleaning MZB data from Miguel Iglesias. 
...
# or 
setwd("~/01_Uni/03_GetReal/02_WP_02/Community Data/02_Miguel_Iglesias/")
samples = readxl::read_excel("01_OriginalData/MACROINVERTEBRADOS.xls") %>% 
  setDT()
...
# seperate indices for pristine sites
parameters = unique(samples$Par?metro)

IASPT = samples[Par?metro == "Indice IASPT"]
IBMWP = samples[Par?metro == "Indice IBMWP"]

names(IASPT)
IASPT = IASPT[, .SD, .SDcols = c(2,9,12)]
names(IASPT) <- c("site", "date", "IASPT")
IASPT$date <- dmy(IASPT$date)
IASPT$IASPT <- as.numeric(IASPT$IASPT)
IASPT = IASPT[, .(IASPT = mean(IASPT, na.rm = T)),  site]
IBMWP = IBMWP[, .SD, .SDcols = c(2,9,12)]
names(IBMWP) <- c("site", "date", "IBMWP")
IBMWP$date <- dmy(IBMWP$date)
IBMWP$IBMWP <- as.numeric(IBMWP$IBMWP)
IBMWP = IBMWP[, .(IBMWP = mean(IBMWP, na.rm = T)),  site]

pristine_data = left_join(IASPT,
                          IBMWP,
                          by = "site") %>% 
        setDT

pristine_data[,c("IASPT_q", "IBMWP_q") := 
                      .(
                              ifelse(IASPT >= 14.5, 1, 0),
                              ifelse(IBMWP >= 14.5, 1, 0)
                      )]
hist(pristine_data$IBMWP)

pristine_data[,prisitne := ifelse(IASPT >= 5, 1,0) ]
...
saveRDS(data2,  "03_FinalData/01_191217_data_before_taxon_clean_mzb_Miguel_Iglesias.RDS")
data2 = readRDS("03_FinalData/01_191217_data_before_taxon_clean_mzb_Miguel_Iglesias.RDS")
...
saveRDS(classification.object, "03_FinalData/02_191217_classification.object_mzb_Miguel_Iglesias.RDS")
classification.object = readRDS("03_FinalData/02_191216_classification.object_mzb_Miguel_Iglesias.RDS")
...
#saveRDS(taxontable, "03_FinalData/03_191217_initial_taxon_clean_mzb_Miguel_Iglesias.RDS")
taxontable = readRDS("03_FinalData/03_191217_initial_taxon_clean_mzb_Miguel_Iglesias.RDS")
...
#saveRDS(taxontable, "03_FinalData/04_191217_post_correction_taxontable_mzb_Miguel_Iglesias.RDS")
taxontable = readRDS("03_FinalData/04_191217_post_correction_taxontable_mzb_Miguel_Iglesias.RDS")


# add taxonomical information to data 
data3 = left_join(data2, taxontable,  by = "taxon") %>%  
  setDT
...
## -- now problematic cases are taken care of. Add the rest automatically 
# redefine yes_order 1. so that entries fixed in add_higher_taxa.R are not checked agaian and 2. to enable piecewise iterative working with the loop, abborting whenever needs fixing. 
yes_order <- sort(data3[!is.na(order) & is.na(kingdom), unique(order)])
length(yes_order)
...
data3[is.na(kingdom)]
data3[taxon == "Orden Anomopoda", c("order", "subclass", "class", "phylum", "kingdom") := 
        .("Diplostraca", "Phyllopoda", "Branchiopoda", "Arthropoda", "Animalia")]
data3[taxon == "Arachnida", c("order", "sublcass", "class", "phylum", "kingdom") := 
        .(NA, NA, "Arachnida", "Arthropoda", "Animalia")]
...
saveRDS(data4, paste0("03_FinalData/05b_",Sys.Date(),"_final_taxon_join_mzb_Miguel_Iglesias_high_phly.RDS"))
data4 = readRDS("      03_FinalData/05b_2020-05-27_final_taxon_join_mzb_Miguel_Iglesias_high_phly.RDS")
...
data4[,gr_sample_id := paste0("site_", site_id2, "_date_", date_id2,"_mzb_Miguel_Iglesias")]
...
# -> only taxonomical clumns have NAs 
...
saveRDS(level_list, paste0("~/01_Uni/03_GetReal/002_WP_02/001_Community Data/001_Landau/03_FinalData/08_",Sys.Date(),"_level_list_MI1.RDS"))
...
st_write(final, paste0("03_FinalData/06_",Sys.Date(),"_MZB_Miguel_Iglesias_higher_taxa.gpkg"))   
saveRDS(final,  paste0("03_FinalData/06_",Sys.Date(),"_MZB_Miguel_Iglesias_higher_taxa.RDS" )) 
